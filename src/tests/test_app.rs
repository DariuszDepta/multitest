...
use crate::test_helpers::{caller, echo, error, hackatom, payout, reflect, TestCustomMsg};
use crate::transactions::{transactional, StorageTransaction};
use crate::wasm::ContractData;
use crate::{
    bail, custom_app, next_block, no_init, AnyResult, App, AppBuilder, AppResponse, Bank,
    CosmosRouter, Distribution, Error, Executor, Module, Router, Staking, Wasm, WasmSudo,
};
...
fn duplicate_contract_code() {
    ...
    let code_id = app.store_code(creator_addr, payout::contract());
...
fn reflect_success() {
    ...
    let mut app = custom_app::<TestCustomMsg, Empty, _>(|router, _, storage| {
        router
            .bank
            .init_balance(storage, &owner, init_funds)
            .unwrap();
    ...
    let creator_addr = app.api().addr_make("creator");
    let payout_id = app.store_code(creator_addr, payout::contract());
    ...
    let creator_addr = app.api().addr_make("creator");
    let reflect_id = app.store_code(creator_addr, reflect::contract());
...
fn send_update_admin_works() {
    ...
    let creator_addr = app.api().addr_make("creator");
    let code_id = app.store_code(creator_addr, hackatom::contract());
...
mod reply_data_overwrite {
    ...
    fn no_submsg() {
        ...
        let creator_addr = app.api().addr_make("creator");
        let code_id = app.store_code(creator_addr, echo::contract());
    ...
    fn single_submsg_reply_returns_none() {
        ...
        // set up reflect contract
        let creator_addr = app.api().addr_make("creator");
        let reflect_id = app.store_code(creator_addr.clone(), reflect::contract());
        ...
        let echo_id = app.store_code(creator_addr, echo::custom_contract());
...
mod custom_messages {
    ...
    fn triggering_custom_msg() {
        let custom_handler = CachingCustomHandler::<TestCustomMsg, Empty>::new();
        ...
        let creator_addr = app.api().addr_make("creator");
        let contract_id = app.store_code(creator_addr, echo::custom_contract());
        ...
        app.execute_contract(
            ...
            contract,
            &echo::Message {
                sub_msg: vec![SubMsg::new(CosmosMsg::Custom(TestCustomMsg::SetAge {
                    age: 20,
                }))],
                ..Default::default()
            },
        ...
        assert_eq!(
            ...
            vec![TestCustomMsg::SetAge { age: 20 }]
        );
...
mod protobuf_wrapped_data {
    ...
    fn instantiate_wrapped_properly() {
        ...
        // set up reflect contract
        let creator_addr = app.api().addr_make("creator");
        let code_id = app.store_code(creator_addr, reflect::contract());
...
mod errors {
    ...
    fn simple_instantiation() {
        ...
        // set up contract
        let creator_addr = app.api().addr_make("creator");
        let code_id = app.store_code(creator_addr, error::contract(false));
    ...
    fn simple_call() {
        ...
        // set up contract
        let creator_addr = app.api().addr_make("creator");
        let code_id = app.store_code(creator_addr, error::contract(true));
    ...
    fn nested_call() {
        ...
        let creator_addr = app.api().addr_make("creator");
        let error_code_id = app.store_code(creator_addr.clone(), error::contract(true));
        let caller_code_id = app.store_code(creator_addr, caller::contract());
